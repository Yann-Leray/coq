...
let check_sort_mask env smask lincheck =
  match smask with
  | PSProp | PSSet -> lincheck
  | PSSProp -> if Environ.sprop_allowed env then lincheck else Type_errors.error_not_allowed_sprop env
  | PSGlobal (_, io)
  | PSType io -> Partial_subst.maybe_add_univ io () lincheck
  | PSQSort (qio, uio) ->
      lincheck
...
and get_holes_profiles_elim env nargs ndecls lincheck = function
  ...
  | PECase (ind, ret, r, brs) ->
      let mib, mip = Inductive.lookup_mind_specif env ind in
      let lincheck = check_sort_mask env r lincheck in
