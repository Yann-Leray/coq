...
let extern_qualuniv uvars : glob_qualuniv option -> qualuniv_expr option = function
  | Some (qo,u) when !print_universes ->
    let qo = Option.map (extern_glob_quality uvars) qo in
    let u = extern_glob_univ uvars u in
    Some (qo,u)
  | _ -> None
...
let rec extern depth0 inctx scopes vars r =
  ...
  | GCases (sty,rtntypopt,tml,eqns) ->
    ...
    let rtntypopt' = Option.map (fun (r, qu) -> extern_typ depth scopes vars' r, extern_qualuniv (snd vars') qu) rtntypopt in
  ...
  | GLetTuple (nal,(na,typopt),tm,b) ->
      ...
      CLetTuple (List.map CAst.make nal,
        (Option.map (fun _ -> (make na)) typopt,
         Option.map (fun (r, qu) -> let vars = add_vname vars na in extern_typ depth scopes vars r, extern_qualuniv (snd vars) qu) typopt),
        sub_extern depth false scopes vars tm,
