...
| id _ Type@{q;u} => Type@{q;u}

| id Type@{q;u} (forall (x : ?A), ?P) => forall x, id Type@{q;u} ?P
| id@{q;} (forall (x : ?A), ?P) ?f => fun (x : ?A) => id@{q;} ?P (?f x)

| id Type@{u} (?A * ?B)%type => (id Type@{u} ?A * id Type@{u} ?B)%type
...
Fail Rewrite Rule id_rew_fail := id (Type@{u} * Type@{u}) (?a, ?b) => (?a, ?b). (* Universe variable u is bound multiple times in the pattern (holes number 0 and 1). *)
