...
sig
  ...
  type 'a case_stk =
    case_info * EInstance.t * 'a array * ('a, ERelevance.t, EQualUniv.t) pcase_return * 'a pcase_invert * ('a, ERelevance.t) pcase_branch array
  type 'a member =
  | App of 'a app_node
...
and apply_rule whrec env sigma ctx psubst es stk =
  ...
  | Declarations.PECase (pind, pret, pqu, pbrs) :: e, Stack.Case ((ci, u, pms, p, iv, brs), cst_l) :: s ->
