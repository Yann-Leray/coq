...
let qualuniv ?loc evd (qo, u : glob_qualuniv) =
  ...
  let evd, u' = glob_univ ?loc evd u in
    ...
    let obj indt rci p v f =
      if not record then
        ...
        let ci = make_case_info !!env (ind_of_ind_type indt) LetStyle in
          ...
          make_case_invert !!env sigma indt ~case_relevance:(EQualUniv.relevance rci) ci, cj.uj_val,[|f|]))
    ...
    let cstrs = get_constructors !!env indf in
      ...
      let sigma, v =
        ...
        mkCase (EConstr.contract_case !!env sigma
                  (ci, (pred, qualuniv),
                   make_case_invert !!env sigma indty ~case_relevance:(EQualUniv.relevance qualuniv) ci, cj.uj_val,
                   [|b1;b2|]))
      in
