...
let judge_of_case env sigma case ci (pj,rp) iv cj lfj =
  ...
  let sigma =
    match Evarconv.unify_leq_delay env sigma (mkSort ps) (mkSort rp) with
    | sigma -> sigma
    | exception Evarconv.UnableToUnify (sigma,e) -> anomaly Pp.(str"Wrong case return universe level")
  in
  let relp = ERelevance.kind sigma @@ ESorts.relevance_of_sort rp in
  ...
  let () = if (match iv with | NoInvert -> false | CaseInvert _ -> true)
              != Inductiveops.Internal.should_invert_case env sigma relp ci
