...
let new_typed_evar env evd sigma ?loc ido (ty, rel) =
  ...
  let evd, c = GlobEnv.new_evar env evd ~src:(loc, knd) ~rrpat:true ~naming (EConstr.of_constr ty) in
  let evk, _ = EConstr.destEvar evd c in
  ...
  let brs = List.fold_left (fun brs eqn ->
    ...
    | { CAst.v = (ids, [pat], rhs); loc } ->
      ...
      | PatVar na ->
        ...
        else WriteOnceArray.fill_remaining (loc, None, rhs) brs
      | PatCstr (cstr, pats, na) ->
        ...
        in
        WriteOnceArray.add i (loc, Some inner_vars, rhs) brs
      end
