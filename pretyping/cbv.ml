...
let uconv info u u' = UGraph.check_eq (Evd.universes info.sigma) u u'
...
and match_qualuniv info pu u psubst =
  match UVars.QualUniv.pattern_match (quconv info) pu u psubst with
  | Some subst -> subst
  | None -> raise PatternFailure
...
and cbv_match_rigid_arg_pattern info env ctx psubst p t =
  ...
  | PHInd (ind, pu), VAL(0, t') ->
    begin match kind t' with Ind (ind', u) when Ind.CanOrd.equal ind ind' -> match_instance info pu u psubst | _ -> raise PatternFailure end
  | PHConstr (constr, pu), CONSTRUCT ((constr', u), [||]) ->
    if Construct.CanOrd.equal constr constr' then match_instance info pu u psubst else raise PatternFailure
  | PHRel i, VAL(k, t') ->
  ...
  | PHSymbol (c, pu), SYMBOL { cst = c', u; _ } ->
    if Constant.CanOrd.equal c c' then match_instance info pu u psubst else raise PatternFailure
  | PHInt i, VAL(0, t') ->
...
and cbv_apply_rule info env ctx psubst es stk =
  ...
  | Declarations.PECase (pind, pu, pret, pqu, pbrs) :: e, CASE (u, pms, (p, qu), brs, iv, ci, env, s) ->
      ...
      let psubst = match_qualuniv info pqu qu psubst in
