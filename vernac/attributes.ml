...
let enabled_rewrite_rules ~proof ?loc orig = function
  | VernacFlagEmpty | VernacFlagLeaf _ ->
    CErrors.user_err ?loc
      Pp.(str "Attribute rewrite_rules only accepts lists of values.")
  | VernacFlagList atts ->
    let rec one res = function
      ...
      | {CAst.v=key, value; loc} :: rem ->
          ...
          if not (get_bool_value ?loc ~key ~default:true value) then
            CErrors.user_err ?loc
              Pp.(str "Attribute rewrite_rules can only enable rules.");
          one (rrl :: res) rem
    in
    let rules = one [] atts in
    let typing_flags = Option.default (Global.typing_flags ()) orig in
    List.fold_left (fun env kn -> (if proof then Environ.enable_rewrite_rules_proof_flags else Environ.enable_rewrite_rules_flags) kn env) typing_flags rules


let typing_flags : Declarations.typing_flags option attribute =
  attribute_of_list
    ["bypass_check", typing_flags_parser;
     "rewrite_rules", enabled_rewrite_rules ~proof:true]
