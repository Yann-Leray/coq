...
let vernac_assumption ~atts kind l inline =
  let DefAttributes.{ scope; polymorphic=poly; program=program_mode; using; user_warns; } = atts in
...
let vernac_fixpoint ~atts ~refine ~pm (rec_order,fixl) =
  let DefAttributes.{ scope; polymorphic=poly; typing_flags; program=program_mode; clearbody; using; user_warns; } = atts in
  if Dumpglob.dump () then
