  ...
  let _, sigma, impls, newfs, _ =
    List.fold_left2
      (fun (env, sigma, uimpls, fields, impls_env) no d ->
         ...
         let qu = EConstr.EQualUniv.make qu in
         let r = EConstr.EQualUniv.relevance qu in
         let impls_env =
           match i with
           | Anonymous -> impls_env
           | Name id ->
...
let typecheck_params_and_fields ~auto_prop_lowering def ~poly udecl ps (records : DataI.t list) : tc_result =
  ...
  let sigma, (newps, ans) =
    ...
    let nf_qualuniv r =
      let r = EConstr.EQualUniv.kind sigma r in
      uvars := Univ.Level.Set.union (snd @@ UVars.QualUniv.levels r) !uvars;
      r
    in
