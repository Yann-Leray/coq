...
type ('a,'s,'u,'r, 'q) univ_visitor = {
  visit_sort : 'a -> 's -> 'a;
  visit_instance : 'a -> 'u -> 'a;
  visit_relevance : 'a -> 'r -> 'a;
  visit_qualuniv : 'a -> 'q -> 'a
}
...
let univs_and_qvars_visitor =
  ...
  in
  let visit_qualuniv (qs, us) qu =
    let q, u = UVars.QualUniv.to_quality_univ qu in
    let qs = let open Sorts.Quality in match q with QVar q -> Sorts.QVar.Set.add q qs | QConstant _ -> qs in
    qs, Universe.levels ~init:us u
  in
  {
