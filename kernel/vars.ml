...
type ('a, 's, 'u, 'qu, 'r) univ_visitor = {
  visit_sort : 'a -> 's -> 'a;
  visit_instance : 'a -> 'u -> 'a;
  visit_qualuniv : 'a -> 'qu -> 'a;
...
let univs_and_qvars_visitor =
  ...
  in
  { visit_sort; visit_instance; visit_qualuniv; visit_relevance }
...
let visit_kind_univs visit acc c =
  ...
  | Array (u,_,_,_) ->
    ...
    acc
  | Case (_, u, _, (_, qu), _, _ ,_) ->
    let acc = visit.visit_instance acc u in
    let acc = visit.visit_qualuniv acc qu in
    acc
  | _ -> acc
