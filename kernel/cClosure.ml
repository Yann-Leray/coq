...
let rec match_main : type a. (a, a patstate) reduction -> _ -> _ -> pat_state:(fconstr, stack, _, a) depth -> _ -> _ -> a =
  ...
  | LocStart { elims; context; head; stack; next = Return _ as next } ->
    ...
    begin match Array.split @@ Array.map2 (fun state elim ->
      let* { subst; equations; rhs } = match state, elim with Live s, Check [] -> Inl s | _ -> Inr (state, elim) in
      ...
      let fequations = List.map (map_pair (fun t -> mk_clos (subst, usubst) t)) equations in
      let* () = if List.for_all (fun (a, b) -> !conv info tab a b) fequations then Inl () else Inr (Dead, Ignore) in
...
let match_symbol red info tab ~pat_state fl (u, b, r) stk =
  ...
  let states, elims = Array.split @@ Array.map
    (fun r ->
      ...
      match subst with
      | Some subst -> Live { subst; equations = r.Declarations.equations; rhs = r.Declarations.rhs }, Check es
      | None -> Dead, Ignore
    ) (Array.of_list r)
  in
