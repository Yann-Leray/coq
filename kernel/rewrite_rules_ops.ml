...
let rec _unify cv_pb infos ?(elim=Elim None) lft1 t1 lft2 t2 evd =
  ...
  (* 2 constants or 2 eliminations *)
  | (FFlex _ | FApp _ | FCaseT _ | FProj _), _ | _, (FFlex _ | FApp _ | FCaseT _ | FProj _) -> raise FailElim
  ...
  | (FRel _ | FAtom _ | FInd _ | FFix _ | FCoFix _ | FCaseInvert _
    | FProd _ | FInt _ | FFloat _ | FString _ | FArray _), _ -> raise NotConvertible
...
let check_inferred_constraints env evd (Info info) =
  ...
  let ug = Environ.universes env
    |> Level.Map.fold (fun lvl _ ug -> UGraph.add_universe lvl ~strict:false ug) evd.univs
...
let test_qvar env nvarqs rel_vars ~rel q =
  ...
  | None ->
      if not @@ Sorts.QVar.Set.mem q (qualities env) then
