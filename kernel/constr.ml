...
type t = T of (t, t, Sorts.t, Instance.t, Sorts.relevance, UVars.QualUniv.t) kind_of_term [@@unboxed]
type constr = t
type types = constr
...
type case = (constr, types, Instance.t, Sorts.relevance, UVars.QualUniv.t) pcase
...
module GenHCons(C:sig
    type t
    val kind : t -> (t, t, Sorts.t, Instance.t, Sorts.relevance, UVars.QualUniv.t) kind_of_term
    val self : t -> constr
    val refcount : t -> int
...
let rec hash_term (t : t) : int * (constr,constr,_,_,_,_) kind_of_term =
  match kind t with
  | Var i ->
  ...
  | Case (ci,u,pms,(p,r),iv,c,bl) ->
    ...
    let hr, r = UVars.QualUniv.hcons r in
...
let rec debug_print c =
  ...
  | Case (_ci,_u,pms,(p,qu),iv,c,bl) ->
    ...
    v 0 (hv 0 (str"Case" ++ brk (1,1) ++
             ...
             spc () ++ str"return"  ++ str"@{" ++ UVars.QualUniv.pr Sorts.QVar.raw_pr Univ.Universe.raw_pr qu ++ str"}" ++ brk (1,1) ++
             pr_ctx p ++ debug_invert iv ++ spc () ++ str"with") ++
